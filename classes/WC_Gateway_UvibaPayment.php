...
class WC_Gateway_UvibaPayment extends WC_Payment_Gateway {
...
if ( $this->settings['show_logo'] != "no" ) { // Check if Show-Logo has been allowed
				...
				//uvibapay_IMG . 'logo_' . $this->settings['show_logo'] . '.png';
			}
 $this->msg['message']	= '';
            $this->msg['class'] 	= '';
			
			add_action('init', array(&$this, 'check_uvibapay_response'));
            add_action('woocommerce_api_' . strtolower(get_class($this)), array($this, 'check_uvibapay_response')); //update for woocommerce >2.0
            ...
            add_action('woocommerce_receipt_uvibapay', array(&$this, 'receipt_page'));	 
...
if($test_mode=='test'){
	...
	if($show_er===true){
		$this->add_admin_notice( 'keys', 'notice notice-error', sprintf( __( 'Uviba is in test mode however your test keys may not be valid. Test keys start with pk_test for public keys and sk_test for private keys (secret keys). Please go to your settings and, <a href="%s">set your UvibaPay account keys</a>.', 'woo_uvibapay' ), $this->get_setting_link() ), true );
	}
}else{
	$show_er=false;
	...
	if($show_er===true){
		$this->add_admin_notice( 'keys', 'notice notice-error', sprintf( __( 'Uviba is in live mode however your live keys may not be valid. Live keys start with pk_live for public keys and sk_live for private keys (secret keys). Please go to your settings and, <a href="%s">set your UvibaPay account keys</a>.', 'woo_uvibapay' ), $this->get_setting_link() ), true );
	}
	...
	'enabled' => array(
		'title' 		=> __('Enable/Disable:', 'woo_uvibapay'),
		'type' 			=> 'checkbox',
		'label' 		=> __('Enable Uviba Payments', 'woo_uvibapay'),
		'default' 		=> 'yes',
		'description' 	=> 'Show in the Payment List as a payment option'
	),
	// Title as displayed on Frontend
	'title' => array(
		'title' 		=> __('Title:', 'woo_uvibapay'),
		'type'			=> 'text',
		'default' 		=> __('Credit Card (Uviba)', 'woo_uvibapay'),
		'description' 	=> __('This controls the title which the user sees during checkout.', 'woo_uvibapay'),
		'desc_tip' 		=> true
	),
// Description as displayed on Frontend
	'description' => array(
		'title' 		=> __('Description:', 'woo_uvibapay'),
		'type' 			=> 'textarea',
		'default' 		=> __("Pay securely with:\n - Credit or Debit Cards\nPowered by Uviba.", 'woo_uvibapay'),
		'description' 	=> __('This controls the description which the user sees during checkout.', 'woo_uvibapay'),
		'desc_tip' 		=> true
	),
	'test_mode' => array(
		'title' 		=> __('Mode:', 'woo_uvibapay'),
		'type' 			=> 'select',
		'label' 		=> __('UvibaPay Test/Live Mode.', 'woo_uvibapay'),
	...
	'button_text' => array(
        ...
        'default' => __( 'Pay with Uviba', 'woocommerce' ),
        'desc_tip'      => true,
    ),
	'redirect_page' => array(
					'title' 			=> __('Return Page'),
					'type' 			=> 'select',
					'options' 		=> $this->uvibapay_get_pages('Select Page'),
					'description' 	=> __('URL of success page', 'woo_uvibapay'),
					'desc_tip' 		=> true
                ),
    'one_dollar' => array(
    ...
    'payments_card' => array(
		'title' 		=> __('Enable/Disable:', 'woo_uvibapay'),
		'type' 			=> 'checkbox',
		'label' 		=> __('Card payments', 'woo_uvibapay'),
		'default' 		=> 'yes',
	),
	'payments_ach' => array(
		'title' 		=> __('Enable/Disable:', 'woo_uvibapay'),
		'type' 			=> 'checkbox',
		'label' 		=> __('Bank payments', 'woo_uvibapay'),
		'default' 		=> 'yes',
	),
	'payments_paypal' => array(
		'title' 		=> __('Enable/Disable:', 'woo_uvibapay'),
		'type' 			=> 'checkbox',
		'label' 		=> __('Paypal payments', 'woo_uvibapay'),
		'default' 		=> 'yes',
	),
    // 'description' => array(
...
public function admin_options(){
			echo '<h3>'.__('Uviba Payment', 'woo_uvibapay').'</h3>';
			echo '<p>'.__('Uviba Payments works by adding payment fields on the checkout and then sending the details to Uviba for verification. <a href=\'https://pay.uviba.com\' >Sign up</a> for a Uviba account, and get your Uviba Payments account keys.', 'woo_uvibapay').'</p>';
		...
		function receipt_page($order){
			...
			echo '<p><strong>' . __('Thank you for your order.', 'woo_uvibapay').'</strong><br/>' . __('The payment page will open soon.', 'woo_uvibapay').'</p>';
			echo $this->generate_uvibapay_form($order);
		} //END-receipt_page
    
        /**
         * Generate button link
         **/
		function generate_uvibapay_form($order_id){
			global $woocommerce;
...
//die($redirect_url);
			...
			$uvibapay_args_array = array();
			foreach($uvibapay_args as $key => $value){
				$uvibapay_args_array[] = "<input type='hidden' name='$key' value='$value'/>";
			}
...
$total_amount_in_dollar = round($uvibapay_args['amount']*100/$this->settings['one_dollar']);
$test_mode = $this->settings['test_mode'];
...
if($this->settings['payments_paypal']=='no'){
	...
	return '<form id="current_uvi_form_id"  action="'.$redirect_url.'" method="post"  >'
.implode('', $uvibapay_args_array).
 '<script 
    ...
    data-amount_string="'.$currency.''.sprintf("%.2f",round($uvibapay_args['amount'],2)).'"
    ...
    top: 2px;
    left: 8px;
" class="button cancel" href="'.$order->get_cancel_order_url().'">'.__('Cancel order &amp; restore cart', 'woo_uvibapay').'</a>
<script type="text/javascript">
UviPay.onButtonReady(function(form_id){
		
try{
	uviPay_openPaymentModal(1);
}catch(e){}

	});</script>
';				
		 
		} //END-generate_uvibapay_form
...
return $response_data;
         ...
         **/
        function check_uvibapay_response(){
            global $woocommerce;
...
if($this->settings['one_dollar']==0){
	$one_dollar=1;
						...
						if(empty($charge_data['charge'])){
							...
							$order->add_order_note('Uviba Payment successfully completed. <br/> ('.$_REQUEST['txnid'].')');
	...
	function uvibapay_get_pages($title = false, $indent = true) {
		...
		public function get_setting_link() {
			...
			$section_slug = $use_id_as_section ? 'uvibapay' : strtolower( 'WC_Gateway_UvibaPayment' );
